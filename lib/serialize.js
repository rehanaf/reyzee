   ...
   const client = Object.defineProperties(hisoka, {
      ...
      sendContact: {
         async value(jid, number, quoted, options = {}) {
            ...
            return hisoka.sendMessage(jid, {
               ...
               mentions: quoted?.participant ? [hisoka.decodeJid(quoted?.participant)] : [hisoka.decodeJid(hisoka?.user?.id)],
               ...options
            }, { quoted, ...options })
         },
         enumerable: true
      ...
      sendGroupV4Invite: {
         async value(jid, groupJid, inviteCode, inviteExpiration, groupName, jpegThumbnail, caption = "Invitation to join my WhatsApp Group", options = {}) {
            const media = await prepareWAMessageMedia({ image: (await Function.getFile(jpegThumbnail)).data }, { upload: hisoka.waUploadToServer })
            ...
            const m = generateWAMessageFromContent(jid, message, { userJid: hisoka.user?.id })
            await hisoka.relayMessage(jid, m.message, { messageId: m.key.id })
